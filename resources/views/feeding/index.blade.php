<x-layouts.app title="Pemberian Pakan">
    <div class="space-y-6">
        <x-page-header title="Pemberian Pakan" description="Kelola riwayat penggunaan pakan, nutrisi, dan obat pada petak budidaya.">
        ...
        <x-card>
            <form method="GET" action="{{ route('feeding.index') }}" class="flex flex-col gap-3 lg:flex-row lg:items-start">
                <div class="min-w-0 flex-1">
                    <label for="search" class="sr-only">Cari pemberian pakan</label>
                    <div class="relative">
                        <x-icon name="search" class="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-neutral-400" />
                        <input id="search" name="search" type="search" value="{{ $filters['search'] }}" placeholder="Cari pemberian pakan..." class="h-10 w-full rounded-lg border border-neutral-200 bg-white pl-9 pr-3 text-sm placeholder:text-neutral-400 hover:border-neutral-300">
                    </div>
                </div>
                <x-filters.panel id="feeding-filters" :active-count="$feedingFilterCount" :open="$errors->any()" class="w-full lg:w-auto lg:shrink-0">
                    <div class="grid gap-4 sm:grid-cols-2">
                        ...
                        <x-filters.select name="feed_item_id" label="Pakan, Nutrisi, atau Obat" :options="$feedingItemOptions" :value="$filters['feedItemId']" placeholder="Semua Kebutuhan" class="sm:col-span-2" />
            ...
            <x-table-wrapper title="Riwayat Pemberian" description="Penggunaan dan biaya dapat diperbarui oleh Admin; transaksi ini tidak mengubah stok bibit.">
                @if ($transactions->isEmpty())
                    <x-empty-state title="Belum ada pemberian pakan" description="Catat penggunaan pakan, nutrisi, obat, atau kebutuhan lainnya." icon="feed">
                        @if (auth()->user()->canAccess('feeding.create'))
                            <x-button :href="route('feeding.create')" data-crud-modal data-crud-modal-size="xl">Tambah Pemberian</x-button>
                        @endif
                    </x-empty-state>
                @else
                    <table data-responsive-table="feeding" class="w-full min-w-[1180px] text-left">
                        <thead>
                            <tr class="border-b border-neutral-200 bg-neutral-50/70 text-[11px] font-semibold uppercase tracking-wider text-neutral-500">
                                ...
                                <th scope="col" class="px-5 py-3 text-center">Pakan / Nutrisi / Obat</th>
                            ...
                            @foreach ($transactions as $transaction)
                                ...
                                <tr class="transition-colors hover:bg-neutral-50/70">
                                    ...
                                    <td class="px-5 py-3.5">
                                        ...
                                        <p class="mt-0.5 text-xs text-neutral-500">{{ $typeLabels[$transaction->feedItem->item_type] }}</p>
                                    </td>
                                    ...
                                    <td class="px-5 py-3.5 pr-6 text-center">
                                        <div class="flex items-center justify-center gap-1" data-transaction-actions>
                                            ...
                                            @if (auth()->user()->canAccess('feeding.delete'))
                                                <form method="POST" action="{{ route('feeding.destroy', $transaction) }}" class="inline-flex" data-confirm="Hapus transaksi Pemberian Pakan {{ $transaction->transaction_number }}?" data-confirm-title="Hapus Transaksi Pemberian Pakan" data-confirm-description="Catatan penggunaan {{ $transaction->feedItem->name }} di {{ $transaction->location->name }} akan dihapus. Stok bibit tidak berubah." data-confirm-action="Hapus Transaksi" data-confirm-tone="danger">
                                                    @csrf
                                                    @method('DELETE')
