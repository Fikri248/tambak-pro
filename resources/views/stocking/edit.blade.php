<x-layouts.app title="Edit Pembibitan">
    <div class="mx-auto max-w-5xl space-y-6">
        <div>
            <a href="{{ route('stocking.show', $stockingTransaction) }}" class="mb-4 inline-flex items-center gap-2 text-sm text-neutral-500 hover:text-neutral-900">
                <x-icon name="arrow-left" class="size-4" />
                Detail Pembibitan
            </a>
            <x-page-header title="Edit Pembibitan" description="Perbarui transaksi {{ $stockingTransaction->transaction_number }} tanpa mengubah identitas pencatatan awal." />
        </div>
        ...
        <div role="note" class="rounded-xl border border-neutral-200 bg-neutral-50 px-4 py-3 text-sm leading-6 text-neutral-700 sm:px-5">
            Sistem akan membatalkan dampak stok lama dan menerapkan perubahan secara atomik. Perubahan data stok akan ditolak jika Batch sudah memiliki aktivitas lanjutan.
        </div>

        <form method="POST" action="{{ route('stocking.update', $stockingTransaction) }}" class="space-y-6" data-stocking-form>
            @csrf
            @method('PATCH')
            ...
            <x-card>
                <div class="grid gap-5 md:grid-cols-2">
                    <x-form.input
                        ...
                        type="datetime-local"
                        :value="old('transaction_date', $stockingTransaction->transaction_date->format('Y-m-d\TH:i'))"
                        required
                    />
                    ...
                    <x-form.select name="location_id" label="Lokasi Petak" required data-summary-source="location">
                        ...
                        @foreach ($locations as $location)
                            <option
                                ...
                                @selected((string) old('location_id', $stockingTransaction->location_id) === (string) $location->id)
                            >
                    ...
                    <x-form.select name="commodity_id" label="Komoditas" required data-summary-source="commodity">
                        ...
                        @foreach ($commodities as $commodity)
                            <option
                                ...
                                @selected((string) old('commodity_id', $stockingTransaction->batch->commodity_id) === (string) $commodity->id)
                            >
                    ...
                    <x-business-code
                        label="Kode Batch"
                        :value="$stockingTransaction->batch->batch_code"
                        data-summary-source="batch"
                        :data-value="$stockingTransaction->batch->batch_code"
                    />
                    ...
                    <div>
                        <x-form.input
                            ...
                            label="Jumlah Bibit"
                            type="number"
                            :value="$stockingTransaction->quantity"
                            min="0.001"
                            step="0.001"
                            placeholder="1000"
                            required
                            data-summary-source="quantity"
                        />
                    ...
                    <x-form.input
                        ...
                        label="Total Biaya"
                        type="number"
                        :value="$stockingTransaction->total_cost"
                        min="0"
                        step="0.01"
                        placeholder="500000"
                        required
                        data-summary-source="cost"
                    />
                    ...
                    <div class="rounded-lg border border-neutral-200 bg-neutral-50 px-4 py-3">
                        ...
                        <p class="mt-1 text-xs text-neutral-500">Dihitung ulang oleh server saat perubahan disimpan.</p>
                    </div>
                    ...
                    <div class="md:col-span-2">
                        <x-form.textarea
                            name="notes"
                            label="Catatan"
                            :value="$stockingTransaction->notes"
                            rows="4"
                            placeholder="Catatan pembelian atau kondisi bibit (opsional)."
                        />
                    </div>
                </div>
            </x-card>
            ...
            <div class="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
                <x-button variant="secondary" :href="route('stocking.show', $stockingTransaction)" data-crud-modal-cancel>Batal</x-button>
